...
class PPLPipeline:
    ...
    def cross_validate_walk_forward(self, X, y, n_splits=3, epochs=5):
        """
        ...
        for train_index, test_index in tscv.split(X):
            ...
            # Train
            model_clone.fit(X_train, y_train, epochs=epochs, batch_size=32, verbose=0)
        ...
        print(f"Average CV Results - MSE: {avg_mse:.5f}, MAE: {avg_mae:.5f}")
    ...
    def run(self, input_cols=None, output_cols=None, sample_size=100, epochs=5, lime_samples=500, shap_samples=200, window_size=32, run_name=None, status_callback=None, df_override=None):
        def log(msg, step=None, total_steps=6):
            print(msg)
            if status_callback:
                status_callback(msg, step, total_steps)
        ...
        mse, mae = self.cross_validate_walk_forward(self.X_images, self.y_ts, epochs=epochs)
        
        # 2. Retrain on Full Data for final artifact
        log("   Retraining on full dataset for final model...")
        es = EarlyStopping(monitor='loss', patience=10, restore_best_weights=True)
        history = self.model.fit(self.X_images, self.y_ts, epochs=epochs, batch_size=32, verbose=0, callbacks=[es])
